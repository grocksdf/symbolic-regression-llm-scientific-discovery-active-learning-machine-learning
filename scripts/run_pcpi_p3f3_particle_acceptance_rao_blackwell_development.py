"""Run the acceptance Rao-Blackwell P3F.3-VR.5 development audit.
...
STAGE = "P3F.3-VR.5"
EXPERIMENT = "open_target_particle_acceptance_rao_blackwell_development"
CONFIG_SCHEMA = "pcpi-p3f3-open-target-particle-acceptance-rao-blackwell-development-v1"
...
CLAIM_BOUNDARY = (
    "This is a conditional acceptance Rao-Blackwell, strict total-evaluation-"
    "matched exact-fixture variance-reduction development audit. It is not "
    "confirmatory fidelity, predictive calibration, real-data "
...
def _evaluate(config: dict[str, Any], target_config: dict[str, Any]) -> dict[str, Any]:
    ...
    if (
        config["mechanism_eligibility"].get(
    ...
    decisions = {
        ...
        and max(
            ...
            for run in completed_candidate
        ) <= thresholds["rao_blackwell_pair_normalization_max_abs_error"],
...
def main() -> int:
    ...
    summary["source_identity"] = {
        ...
        "acceptance_rao_blackwell_config_sha256": file_sha256(
            args.config.resolve()
        ),
    ...
    (output / "acceptance_rao_blackwell_config.json").write_text(
