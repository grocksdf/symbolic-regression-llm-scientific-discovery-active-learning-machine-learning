"""Run the full-population waste-free P3F.3-VR.3 development audit.
...
carry N states into every subsequent potential, use the same fixed beta grid,
and match incremental-potential, MH proposal/target, posterior-functional, and
total target-evaluation budgets.
...
STAGE = "P3F.3-VR.3"
EXPERIMENT = "open_target_particle_variance_reduction_full_population_development"
CONFIG_SCHEMA = (
    "pcpi-p3f3-open-target-particle-variance-reduction-full-population-development-v3"
)
...
CLAIM_BOUNDARY = (
    "This is a full-population, strict total-evaluation-matched exact-fixture "
...
def _run_one(
    ...
    expected_proposals_per_bridge = int(
    ...
    expected_proposals = expected_proposals_per_bridge * len(diagnostics)
    ...
    return {
        **base,
        ...
        "waste_free_pool_states": (
            particle_count * len(diagnostics)
            if particle_config.method_id == WASTE_FREE_METHOD
            else 0
        ),
...
def _evaluate(config: dict[str, Any], target_config: dict[str, Any]) -> dict[str, Any]:
    ...
    if expected_proposals != population_size or expected_potentials != population_size:
    ...
    if expected_bridges * expected_proposals != config["matched_budget"][
    ...
    decisions = {
        ...
        and all(
            ...
            == config["matched_budget"]["full_population_size"] * expected_bridges
...
def main() -> int:
    ...
    summary["source_identity"] = {
        ...
        "variance_reduction_full_population_config_sha256": file_sha256(
            args.config.resolve()
        ),
