"""Run the observation-terminal waste-free P3F.3-VR.4 development audit.

The runner compares standard N-particle one-step SMC with canonical waste-free
SMC using M source chains and P retained post-MH states, N=M*P.  Both methods
carry N states into every subsequent potential and use the same fixed beta
grid. Resample-move is frozen to beta=1 once per observation; incremental-
potential, MH proposal/target, posterior-functional, and total target-
evaluation budgets remain exactly matched.
...
STAGE = "P3F.3-VR.4"
EXPERIMENT = "open_target_particle_variance_reduction_observation_terminal_development"
CONFIG_SCHEMA = (
    "pcpi-p3f3-open-target-particle-variance-reduction-observation-terminal-development-v4"
)
...
CLAIM_BOUNDARY = (
    "This is an observation-terminal, full-population, strict total-evaluation-"
    "matched exact-fixture variance-reduction development "
...
def _run_one(
    ...
    waste_free_resampling_events = sum(
    ...
    return {
        **base,
        ...
        == config["matched_budget"]["full_population_size"],
        "posterior_estimator_particle_count": len(result.posterior_particles),
        "posterior_estimator_kind": (
            "waste-free-full-propagating-population"
            if particle_config.method_id == WASTE_FREE_METHOD
            else "standard-full-propagating-population"
        ),
        ...
        "waste_free_pool_states": (
            particle_count * expected_rejuvenation_events
            if particle_config.method_id == WASTE_FREE_METHOD
            else 0
        ),
        "waste_free_retained_states": (
            particle_count * expected_rejuvenation_events
            if particle_config.method_id == WASTE_FREE_METHOD
            else 0
        ),
...
def _method_aggregates(runs: list[dict[str, Any]]) -> dict[str, Any]:
    output: dict[str, Any] = {}
    fields = ERROR_FIELDS + GENEALOGY_FIELDS + MIXING_FIELDS + (
...
def _paired_rows(runs: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for (fixture_id, seed), methods in sorted(groups.items()):
        if set(methods) != {STANDARD_METHOD, WASTE_FREE_METHOD}:
            continue
        baseline = methods[STANDARD_METHOD]
        candidate = methods[WASTE_FREE_METHOD]
        row: dict[str, Any] = {"fixture_id": fixture_id, "seed": seed}
        row[f"bridge_count::{STANDARD_METHOD}"] = baseline["bridge_count"]
        row[f"bridge_count::{WASTE_FREE_METHOD}"] = candidate["bridge_count"]
        row[f"proposal_evaluations::{STANDARD_METHOD}"] = baseline[
            "proposal_evaluations"
        ]
        row[f"proposal_evaluations::{WASTE_FREE_METHOD}"] = candidate[
            "proposal_evaluations"
        ]
        ...
        for field in ERROR_FIELDS:
            ...
            row[f"{field}::{WASTE_FREE_METHOD}"] = candidate[field]
            row[f"{field}::improvement"] = baseline[field] - candidate[field]
        row[f"log_evidence_signed_error::{STANDARD_METHOD}"] = baseline[
            "log_evidence_exact_reference_signed_error"
        ]
        row[f"log_evidence_signed_error::{WASTE_FREE_METHOD}"] = candidate[
            "log_evidence_exact_reference_signed_error"
        ]
        row[f"maximum_ancestry_log_attrition_per_event::{STANDARD_METHOD}"] = baseline[
            "maximum_ancestry_log_attrition_per_resampling_event"
        ]
        row[f"maximum_ancestry_log_attrition_per_event::{WASTE_FREE_METHOD}"] = candidate[
            "maximum_ancestry_log_attrition_per_resampling_event"
        ]
        row[f"maximum_root_entropy_loss_per_event::{STANDARD_METHOD}"] = baseline[
            "maximum_root_entropy_loss_per_resampling_event"
        ]
        row[f"maximum_root_entropy_loss_per_event::{WASTE_FREE_METHOD}"] = candidate[
            "maximum_root_entropy_loss_per_resampling_event"
        ]
        rows.append(row)
    return rows
...
def _evaluate(config: dict[str, Any], target_config: dict[str, Any]) -> dict[str, Any]:
    ...
    if any(
        item.population_size != config["matched_budget"]["full_population_size"]
    ...
    completed_candidate = [
        ...
        if run["method_id"] == WASTE_FREE_METHOD
    ]
    ...
    candidate = aggregates.get(WASTE_FREE_METHOD, {})
    ...
    decisions = {
        ...
        "candidate_full_population_propagated": candidate_ready
        and all(
            run["posterior_estimator_particle_count"]
            == config["matched_budget"]["full_population_size"]
            and run["posterior_estimator_kind"]
            == "waste-free-full-propagating-population"
            and run["waste_free_retained_states"]
            == config["matched_budget"]["full_population_size"]
            * expected_rejuvenation_events
        ...
        "candidate_raw_per_resampling_event_ancestry": candidate_ready
        and worst(candidate, "maximum_ancestry_log_attrition_per_resampling_event")
        <= thresholds["candidate_raw_ancestry_log_attrition_per_event_max"],
        "candidate_raw_per_resampling_event_root_entropy": candidate_ready
        and worst(candidate, "maximum_root_entropy_loss_per_resampling_event")
        <= thresholds["candidate_raw_root_entropy_loss_per_event_max"],
        "candidate_capacity_adjusted_per_resampling_event_ancestry": candidate_ready
        and worst(
            candidate,
            "maximum_capacity_adjusted_ancestry_log_attrition_per_resampling_event",
        ) <= thresholds["candidate_capacity_adjusted_ancestry_log_attrition_max"],
        "candidate_capacity_adjusted_per_resampling_event_root_entropy": candidate_ready
        and worst(
            candidate,
            "maximum_capacity_adjusted_root_entropy_loss_per_resampling_event",
        ) <= thresholds["candidate_capacity_adjusted_root_entropy_loss_max"],
        "candidate_terminal_raw_root_fraction": candidate_ready
        and minimum(candidate, "terminal_distinct_root_ancestor_fraction")
        >= thresholds["candidate_terminal_distinct_root_ancestor_fraction_min"],
        "candidate_terminal_raw_root_entropy": candidate_ready
        and minimum(candidate, "terminal_normalized_root_entropy")
        >= thresholds["candidate_terminal_normalized_root_entropy_min"],
        "candidate_terminal_capacity_adjusted_root_fraction_noninferior": baseline_ready
        and candidate_ready
        and _safe_ratio(
            minimum(candidate, "terminal_capacity_adjusted_root_ancestor_fraction"),
            minimum(baseline, "terminal_distinct_root_ancestor_fraction"),
        ) >= thresholds["candidate_terminal_capacity_adjusted_root_fraction_ratio_min"],
        "candidate_terminal_capacity_adjusted_root_entropy_noninferior": baseline_ready
        and candidate_ready
        and _safe_ratio(
            minimum(candidate, "terminal_capacity_adjusted_root_entropy"),
            minimum(baseline, "terminal_normalized_root_entropy"),
        ) >= thresholds["candidate_terminal_capacity_adjusted_root_entropy_ratio_min"],
    }
...
def main() -> int:
    ...
    summary["source_identity"] = {
        ...
        "variance_reduction_observation_terminal_config_sha256": file_sha256(
            args.config.resolve()
        ),
