"""Run only the response-free CERT.6 resident common-target proof checks."""
...
EXPECTED_CHECKS = {
    ...
    "tests/test_pcpi_p3f4_resident_common_target.py": (
        "test_resident_fixed_shell_and_open_prior_draws_have_no_uint64_ceiling",
        "test_resident_semantic_design_is_exactly_alias_constant_by_construction",
        "test_resident_particle_construction_uses_semantic_not_raw_cache_identity",
        "test_common_target_adapter_preserves_exact_endpoints_ratio_and_balance",
        "test_common_target_plan_is_fail_closed_and_does_not_authorize_resident_smc",
        "test_common_target_adapter_rejects_raw_or_nonfinite_target_shortcuts",
        "test_cert6_adapter_has_no_resident_smc_import_or_execution_surface",
    ),
}
...
def main() -> int:
    ...
    print(
        json.dumps(
            {
                "schema": (
                    "pcpi-p3f4-cert6-resident-common-target-response-free-checks-v1"
                ),
                "status": "passed",
                ...
                "role": (
                    "response-free exact resident support, semantic-target, and "
                    "involutive-transition adapter audit"
                ),
                "resident_arbitrary_precision_open_support_verified": True,
                "resident_semantic_target_identity_verified": True,
                "resident_common_target_adapter_verified": True,
