...
| P3E.2 numerical reproducibility | Canonical fixture initially varied in strict union-orthogonality error across BLAS/NumPy environments; fresh canonical artifact records `4.44e-16` | Near-null eigensolver modes leaked tiny components into the structure union; this is a floating-point contract issue, not posterior or utility evidence | **No: separate task-independent reprojection repair** | Reprojection repair is implemented; commit `ca4bfce8` and the fresh 11/11 P3E.2 run close source identity, while real adequacy remains untested |
