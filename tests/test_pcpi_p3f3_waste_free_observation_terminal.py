"""Correctness and boundary tests for observation-terminal P3F.3-VR.4."""
...
RUNNER_PATH = (
    ROOT
    / "scripts/run_pcpi_p3f3_particle_variance_reduction_observation_terminal_development.py"
)
...
def test_full_population_methods_match_every_registered_evaluation_budget() -> None:
    ...
    rejuvenation_event_count = len(targets)
    for result in results.values():
        assert len(result.particles) == 64
        assert len(result.posterior_particles) == 64
        assert len(result.diagnostics) == bridge_count
        assert len(result.resampling_genealogy) == rejuvenation_event_count
        assert (
            sum(item.proposals for item in result.diagnostics)
            == 64 * rejuvenation_event_count
        )
        for item in result.diagnostics:
            if item.beta_current == 1.0:
                assert item.proposals == 64
                assert item.resampled
            else:
                assert item.proposals == 0
                assert not item.resampled
                assert item.resampling_reason == "none"
        assert math.isclose(
...
def test_vr4_bank_is_new_and_does_not_freeze_confirmatory_data() -> None:
    ...
    assert config["stage"] == "P3F.3-VR.4"
    ...
    assert set(config["seeds"]).isdisjoint(vr3["seeds"])
    ...
    assert fixture_ids.isdisjoint(
        {fixture["fixture_id"] for fixture in vr3["fixtures"]}
    )
    assert fixture_ids.isdisjoint(
